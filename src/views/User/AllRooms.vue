    ...
    <div class="row g-4">
            ...
            <div class="mb-2">
              ...
              <div class="amenity-filter-grid">
                <div
                  ...
                  :key="opt.id"
                  class="amenity-check-item"
                  :class="{ active: filters.options.includes(opt.id) }"
      ...
      <div class="col-lg-9">
        ...
        <div v-else class="row g-3">
          <RouterLink to="/rooms/:id"></RouterLink>
            <div
              v-for="room in paginatedRooms"
              :key="room.id"
              class="col-md-6 col-xl-4"
            >
              <div class="room-card card border-0 shadow-sm rounded-4 overflow-hidden h-100" @click="$router.push(`/rooms/${room.id}`)">
                ...
                <div class="card-body p-3">
                  ...
                  <div class="d-flex flex-wrap gap-2 mb-2">
                    <span v-if="room.bed" class="spec-chip">
                      <i class="bi bi-person-fill me-1"></i>{{ room.bed }}
                    </span>
                    <span v-if="room.size_room" class="spec-chip">
                  ...
                  <div class="d-flex flex-wrap gap-1">
                    <span
                      v-for="opt in (room.room_options || []).slice(0, 3)"
                      :key="opt.id"
                      class="amenity-tag"
                    >{{ opt.name }}</span>
                    <span
                      v-if="(room.room_options || []).length > 3"
                      class="amenity-tag text-muted"
                    >+{{ room.room_options.length - 3 }} more</span>
                  </div>
                </div>

                <!-- Card footer: bills preview -->
...
const filteredRooms = computed(() => {
  ...
  if (filters.bed) {
    if (filters.bed === '5+') {
      ...
      list = list.filter(r => String(r.bed) === filters.bed || r.bed?.toString().startsWith(filters.bed))
    }
  }
...
onMounted(async () => {
  await Promise.all([
    ...
    roomStore.fetchRooms('sort_col=id&sort_dir=desc&per_page=100'),
  ])
})
